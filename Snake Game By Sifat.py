...
""" Snake Game Author: Md. Sifat Bin Mahbub"""
...
SCREEN = WIDTH * 30, WIDTH * 15  # Screen size in pixels.
FONT = "ms 70"
...
class Game(tk.Frame):
    ...
    def next(self):
        ...
        self.master.title("Snake Game Created By Md. Sifat Bin Mahbub - Score: %d" % self.score)
        if self.snake.is_valid():
            self.after(SPEED, self.next)
        else:
            self.can.create_text(SCREEN[0] // 2, SCREEN[1] // 2,
                                 fill=TEXT_COLOR, font=FONT,
                                 text="'Sifat' Scored: %d" % self.score
                                 )
...
def main():
    root = tk.Tk()
    root.title("Snake Game Created By Sifat")
    root.resizable(0, 0)
    frame = Game(root)
    frame.pack()
...
if __name__ == "__main__":
    main()
# This code is encoded by Md. Sifat Bin Mahbub !!!
